...
from pybind import Class, Module
from pybind.exceptions import CppException
from pybind.parameters import (
    ArgvObjects,
    ClassParameter,
    IntegralArgv,
...
ex = mod.AddException(CppException("LczeroException", cpp_name="lczero::Exception"))
...
input.AddMethod("set_mask").AddParameter(
    NumericParameter("plane"), NumericParameter("mask", type="u64")
).AddEx(ex)
input.AddMethod("set_val").AddParameter(
    NumericParameter("plane"), NumericParameter("value", type="f32")
).AddEx(ex)
...
# Output class
output = mod.AddClass(
    Class("Output", cpp_name="lczero::python::Output", disable_constructor=True)
)
...
output.AddMethod("p_raw").AddParameter(IntegralArgv("samples", "i")).AddRetVal(
    IntegralTupleRetVal("f32")
).AddEx(ex)
output.AddMethod("p_softmax").AddParameter(IntegralArgv("samples", "i")).AddRetVal(
    IntegralTupleRetVal("f32")
).AddEx(ex)
...
backend.AddMethod("evaluate").AddParameter(ArgvObjects("inputs", input)).AddRetVal(
    ObjTupleRetVal(output)
).AddEx(ex)
backend.AddMethod("capabilities").AddRetVal(ObjCopyRetval(backend_caps))

# PositionHistory class
game_state = mod.AddClass(Class("GameState", cpp_name="lczero::python::GameState"))
...
game_state.AddMethod("as_input").AddParameter(
    ClassParameter(backend, "backend", optional=False)
).AddRetVal(ObjOwnerRetval(input)).AddEx(ex)
