...
class Class(FunctionContainer):
    ...
    def GenerateRegister(self, w):
        w.Write(
            f"if (PyType_Ready(&{self.type_object_name()}) " "!= 0) return nullptr;"
        )
        w.Write(
    ...
    def _generate_class_struct(self, w, module):
        ...
        w.Write(
            ".tp_dealloc = reinterpret_cast<destructor>" f"({self.destructor_name()}),"
        )
        w.Write(".tp_flags = Py_TPFLAGS_DEFAULT,")
        w.Write(f".tp_doc = {self.BuildDocString()},")
        w.Write(f".tp_methods = {self.function_list_name()},")
        w.Write(".tp_init = reinterpret_cast<initproc>" f"({self.constructor_name()}),")
    ...
    def _generate_destructor(self, w):
        w.Open(
            f"void {self.destructor_name()}(" f"{self.object_struct_name()}* self) {{"
        )
